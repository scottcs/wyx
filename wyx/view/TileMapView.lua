...
local MapView = getClass 'wyx.view.MapView'
...
local StairMapType = getClass 'wyx.map.StairMapType'
local TrapMapType = getClass 'wyx.map.TrapMapType'
local MapUpdateFinishedEvent = getClass 'wyx.event.MapUpdateFinishedEvent'
local TileMapNodeView = getClass 'wyx.view.TileMapNodeView'
local AnimatedTile = getClass 'wyx.view.AnimatedTile'
...
function TileMapView:destroy()
	...
	for i in ipairs(self._animatedDrawTiles) do
		self._animatedDrawTiles[i] = nil
	end
	self._animatedDrawTiles = nil
...
function TileMapView:setViewport(rect)
	...
	local num = #self._animatedDrawTiles
	for i=1,num do self._animatedDrawTiles[i] = nil end
	...
	for i=1,num do
		...
		if nil ~= color then
			self._drawTiles[count] = {tile=t, color=color}
			count = count + 1
		end
	end
	...
	for i=1,num do
		...
		local color = self:_shouldDraw(t)
		if nil ~= color then
			self._animatedDrawTiles[count] = {tile=t, color=color}
			count = count + 1
		end
	end
